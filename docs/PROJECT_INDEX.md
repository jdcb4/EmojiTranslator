...
EmojiTranslator is a deterministic movie-title-to-emoji converter for quiz clue generation. It uses static JSON tables and transparent rules, with an optional Hono API endpoint.
...
- Runtime/deployment target: browser web app plus Hono API for Cloudflare Workers.
- Persistence: static JSON data files in `src/data/converter`; no database.
